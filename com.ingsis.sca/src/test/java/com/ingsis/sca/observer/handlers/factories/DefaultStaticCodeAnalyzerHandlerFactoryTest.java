...
class DefaultStaticCodeAnalyzerHandlerFactoryTest {
    ...
    void createsHandlersAndTheyReturnCorrectOnDefaultNodes() {
        ...
        com.ingsis.nodes.expression.literal.LiteralNode condLit = new com.ingsis.nodes.expression.literal.LiteralNode("true", 1, 1);
        assertTrue(cond.handle(new IfKeywordNode(condLit, java.util.List.of(), java.util.List.of(), 1,1)).isCorrect());
        assertTrue(expr.handle(new com.ingsis.nodes.expression.literal.LiteralNode("5",1,1)).isCorrect());

        // Declaration handler expects identifier pattern; provide a valid identifier to avoid incorrect
        IdentifierNode id = new IdentifierNode("validName", 1, 1);
        com.ingsis.nodes.expression.literal.LiteralNode literal = new com.ingsis.nodes.expression.literal.LiteralNode("5",1,1);
        com.ingsis.nodes.expression.operator.ValueAssignationNode va = new com.ingsis.nodes.expression.operator.ValueAssignationNode(id, literal, 1,1);
