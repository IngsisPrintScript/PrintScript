...
public class LineExpressionParser implements Parser<ExpressionNode> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> parse(TokenStream stream) {
    ...
    if (processExpressionResult.isUninitialized()) {
      ...
      return ProcessCheckpoint.INITIALIZED(processExpressionResult.iterator(), processExpressionResult.result());
    }
    ...
    if (!consumeSemicolonResult.isCorrect()) {
      return ProcessCheckpoint.INITIALIZED(stream, ProcessResult.PREFIX(NodePriority.EXPRESSION.priority()));
    }
