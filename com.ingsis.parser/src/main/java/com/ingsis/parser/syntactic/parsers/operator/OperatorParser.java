...
public final class OperatorParser implements Parser<ExpressionNode> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> parse(TokenStream stream) {
    ...
    ProcessCheckpoint<Token, ExpressionNode> getExpressionResult = postfixToAstBuilder.build(
        leafParserSupplier.get(),
        getTokensQueueResult.result());
    ...
    return ProcessCheckpoint.INITIALIZED(
        ...
        ProcessResult.COMPLETE(
            getExpressionResult.result(),
            NodePriority.OPERATOR.priority()));
  }
}
