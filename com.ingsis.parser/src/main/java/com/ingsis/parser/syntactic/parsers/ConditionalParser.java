...
public class ConditionalParser implements Parser<Node> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<Node>> parse(TokenStream stream) {
    ...
    ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> conditionCheckpoint = conditionParser.parse(stream);
    ...
    if (elseResult.isCorrect()) {
      ...
      ProcessCheckpoint<Token, ProcessResult<List<Node>>> elseBlockResult = parseBlock(stream);
  ...
  private ProcessCheckpoint<Token, ProcessResult<List<Node>>> parseBlock(TokenStream stream) {
    ...
    while (!stream.consume(rBraceTemplate).isCorrect()) {
      ProcessCheckpoint<Token, ProcessResult<Node>> statementCheckpoint = statementParserSupplier.get().parse(stream);
