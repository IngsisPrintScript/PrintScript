...
public class CallFunctionParser implements Parser<ExpressionNode> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> parse(TokenStream stream) {
    ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> identifierResult = identifierParser.parse(stream);
    if (identifierResult.isUninitialized()) {
      return ProcessCheckpoint.UNINITIALIZED();
    } else if (!identifierResult.result().isComplete()) {
      return ProcessCheckpoint.INITIALIZED(stream, ProcessResult.PREFIX(NodePriority.CALL_FUNCTION.priority()));
    }
    ...
    ProcessCheckpoint<Token, ProcessResult<List<ExpressionNode>>> argumentsResult = parseArguments(stream);
    ...
    if (argumentsResult.isUninitialized()) {
      ...
      return ProcessCheckpoint.INITIALIZED(
          argumentsResult.iterator(), ProcessResult.PREFIX(NodePriority.CALL_FUNCTION.priority()));
    }
  ...
  private ProcessCheckpoint<Token, ProcessResult<List<ExpressionNode>>> parseArguments(TokenStream stream) {
    ...
    if (rParenCheck.isCorrect()) {
      ...
      return ProcessCheckpoint.INITIALIZED(
          stream,
          ProcessResult.COMPLETE(arguments, priority));
    }

    do {
      ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> argumentResult = expressionParserSupplier.get()
          .parse(stream);
      ...
      if (!argumentResult.result().isComplete()) {
        return ProcessCheckpoint.INITIALIZED(
            stream, ProcessResult.PREFIX(priority));
      }
