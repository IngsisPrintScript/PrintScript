...
public class InMemoryParserFactory implements ParserFactory {
  ...
  @Override
  public Parser<Node> conditionalParser(Parser<ExpressionNode> conditionParser, Supplier<Parser<Node>> programParser) {
  ...
  @Override
  public Parser<Node> declarationParser(Parser<ExpressionNode> identifierParser,
  ...
  @Override
  public Parser<ExpressionNode> operatorParser(Supplier<Parser<ExpressionNode>> leafParserSupplier) {
    ShuntingYardStorage storage = new InMemoryShuntingYardStorage(
        tokenTemplateFactory.separator(TokenType.LPAREN.lexeme()).result(),
        tokenTemplateFactory.separator(TokenType.RPAREN.lexeme()).result());
    ShuntingYardTransformer shuntingYardTransformer = new DefaultShuntingYardTransformer(storage);
    PostfixToAstBuilder postfixToAstBuilder = new PostfixExpressionTreeBuilder(
        this.nodeFactory,
        this.tokenFactory,
  ...
  @Override
  public Parser<ExpressionNode> callFunctionParser(Supplier<Parser<ExpressionNode>> leafParserSupplier) {
  ...
  public Parser<ExpressionNode> numberLiteralParser() {
    return new NumberLiteralParser(
        this.tokenTemplateFactory,
        this.nodeFactory);
  }
  ...
  public Parser<ExpressionNode> booleanLiteralParser() {
    return new BooleanLiteralParser(
        this.tokenTemplateFactory,
        this.nodeFactory);
  }
