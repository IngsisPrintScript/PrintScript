...
public class InMemoryShuntingYardStorage implements ShuntingYardStorage {
  ...
  public InMemoryShuntingYardStorage(TokenTemplate leftParenthesisTemplate, TokenTemplate rightParenthesisTemplate) {
    this(new ArrayDeque<Token>(), new ArrayDeque<>(), leftParenthesisTemplate, rightParenthesisTemplate);
  }

  private InMemoryShuntingYardStorage(Deque<Token> operatorsStack, Queue<Token> valuesQueue,
  ...
  private Result<PopData> popHigherOrEqualPrecedenceOperators(
      Deque<Token> originalStack,
      Queue<Token> originalValues,
  ...
  private Result<ShuntingYardStorage> withStackAndQueu(Deque<Token> operators, Queue<Token> values) {
    return new CorrectResult<>(
        new InMemoryShuntingYardStorage(
            operators,
            values,
  ...
  private Result<ShuntingYardStorage> withOperator(Token token) {
    ...
    Result<PopData> popResult = popHigherOrEqualPrecedenceOperators(operatorsStack, valuesQueue, newOperatorType);
  ...
  private Result<PopData> popUntilLeftParenthesis(Deque<Token> originalStack, Queue<Token> originalValues) {
    Deque<Token> stackCopy = new ArrayDeque<>(originalStack);
    Queue<Token> valuesCopy = new ArrayDeque<>(originalValues);
