  ...
  private ParseResult<ExpressionNode> parseTail(
    ...
    return switch (stream.consume()) {
      case ConsumeResult.CORRECT C -> {
        Result<OperatorType> getOperatorType = OperatorType.fromSymbol(C.consumedToken().value());
      ...
      case ConsumeResult.INCORRECT I -> new ParseResult.COMPLETE<>(lhs, stream, nodePriority, true);
    };
  }
}
