...
public class DeclarationParser implements Parser<Node> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<Node>> parse(TokenStream stream) {
    ...
    ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> processExpressionResult = expressionParser.parse(exprSlice);
  ...
  private TokenStream sliceExpression(TokenStream stream) {
    ...
    List<Token> slice = cleanStart.tokens().subList(cleanStart.pointer(), cleanStart.pointer() + offset);
