...
public class DeclarationParser implements Parser<Node> {
  ...
  public ProcessCheckpoint<Token, ProcessResult<Node>> parse(TokenStream stream) {
    ...
    ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> processIdentifierResult = identifierParser.parse(stream);
    if (processIdentifierResult.isUninitialized()) {
      ...
      return ProcessCheckpoint.INITIALIZED(
          processIdentifierResult.iterator(), ProcessResult.PREFIX(NodePriority.STATEMENT.priority()));
    ...
    ProcessCheckpoint<Token, ProcessResult<ExpressionNode>> processExpressionResult = expressionParser.parse(stream);
    if (processExpressionResult.isUninitialized()) {
      ...
      return ProcessCheckpoint.INITIALIZED(
          processExpressionResult.iterator(), ProcessResult.PREFIX(NodePriority.STATEMENT.priority()));
    }
