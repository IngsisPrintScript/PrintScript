...
public final class OperatorNodeValidityHandler implements NodeEventHandler<Node> {
    ...
    public CheckResult handle(Node node, SemanticEnvironment env) {
        if (node instanceof OperatorNode operatorNode) {
            ...
            Types expectedType =
                    new DefaultExpressionTypeGetter().getType(operatorNode.children().get(0), env);
    ...
    private CheckResult recursiveCheck(
            Types expectedType, ExpressionNode node, SemanticEnvironment env) {
    ...
    private CheckResult checkLiteral(
            Types expectedType, LiteralNode literalNode, SemanticEnvironment env) {
        boolean checkResult =
                new DefaultLiteralTypeGetter()
                        .getType(literalNode, env)
    ...
    private CheckResult checkIdentifier(
            Types expectedType, IdentifierNode identifierNode, SemanticEnvironment env) {
        boolean checkResult =
                new DefaultIdentifierTypeGetter()
    ...
    private CheckResult checkFunction(
            Types expectedType, CallFunctionNode callFunctionNode, SemanticEnvironment env) {
        boolean check =
                new DefaultFunctionTypeGetter()
