...
public final class DefaultEnvironment implements Environment {
  ...
  public DefaultEnvironment(
      EntryFactory entryFactory, Map<String, VariableEntry> variables, Map<String, FunctionEntry> functions,
      Environment father) {
  ...
  @Override
  public Result<FunctionEntry> createFunction(String identifier, Map<String, Types> arguments, Types returnType) {
    if (isFunctionDeclared(identifier)) {
      return new IncorrectResult<>("Function is already declared.");
    }
    FunctionEntry functionEntry = entryFactory.createFunctionEntry(returnType, arguments, null);
  ...
  public Result<FunctionEntry> updateFunction(String identifier, List<ExpressionNode> body) {
    ...
    FunctionEntry newFunctionEntry = entryFactory.createFunctionEntry(oldFunctionEntry.returnType(),
        oldFunctionEntry.arguments(), body);
  ...
  public Boolean isFunctionInitialized(String identifier) {
    return isFunctionDeclared(identifier)
        && (isFunctionInitializedHere(identifier) || father().isFunctionInitialized(identifier));
  }
