...
public final class ProgramInterpreter implements SafeIterator<String> {
  ...
  public SafeIterationResult<String> next() {
    ...
    InterpretResult interpretResult = iterationResult.iterationResult().acceptInterpreter(interpreter, evalState);
    return switch (interpretResult) {
      case InterpretResult.INCORRECT I -> iterationResultFactory.createIncorrectResult(I.error());
      case InterpretResult.CORRECT C -> createCorrectResult(C, iterationResult.nextIterator());
    };
  }

  private SafeIterationResult<String> createCorrectResult(InterpretResult.CORRECT result,
      SafeIterator<Interpretable> nextIterator) {
