...
public class Syntactic implements SyntacticInterface {
    ...
    private final PeekableIterator<TokenInterface> TOKEN_ITERATOR;
    private final Deque<SemanticallyCheckable> NODE_CACHE;

    @SuppressFBWarnings("EI_EXPOSE_REP2")
    public Syntactic(ASTreeBuilderInterface treeBuilder, PeekableIterator<TokenInterface> tokenIterator) {
    ...
    private SemanticallyCheckable computeNext() {
        ...
        while (TOKEN_ITERATOR.hasNext()) {
            TokenInterface token = TOKEN_ITERATOR.peek();
