...
public record ScopedEnviroment(Environment parent, Map<String, Binding> bindings)
    ...
    public Optional<Binding> lookup(String identifier) {
        ...
        return parent != null ? parent.lookup(identifier) : Optional.empty();
    }
    ...
    public Environment update(String identifier, Value value) {
        if (bindings.containsKey(identifier)) {
            Map<String, Binding> newBindings = new HashMap<>(bindings);
            Binding.VariableBinding originalBinding =
                    (Binding.VariableBinding) newBindings.get(identifier);
            newBindings.put(
                    identifier,
                    new Binding.VariableBinding(
                            originalBinding.type(),
                            originalBinding.isMutable(),
