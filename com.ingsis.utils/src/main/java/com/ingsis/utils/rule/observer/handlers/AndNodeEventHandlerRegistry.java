...
public class AndNodeEventHandlerRegistry<T extends Node> implements NodeEventHandlerRegistry<T> {
    private final List<NodeEventHandler<T>> handlers;

    public AndNodeEventHandlerRegistry(List<NodeEventHandler<T>> handlers) {
    ...
    public NodeEventHandlerRegistry<T> register(NodeEventHandler<T> newHandler) {
        ...
        return new AndNodeEventHandlerRegistry<>(newHandlers);
    }
}
