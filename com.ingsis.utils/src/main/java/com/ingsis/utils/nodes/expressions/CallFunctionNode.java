...
public record CallFunctionNode(
    ...
    public Result<Object> solve() {
        ...
        Result<FunctionEntry> getFunction =
                runtime.getCurrentEnvironment().readFunction(identifierNode.name());
        ...
        try {
            ...
            for (int i = 0; i < args.size(); i++) {
                ...
                if (!created.isCorrect()) {
                    Result<VariableEntry> updateResult =
                            functionEntry.closure().updateVariable(paramName, argumentValue);
