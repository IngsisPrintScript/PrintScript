    ...
    return switch (left) {
      case Value.IntValue LI -> switch (right) {
        case Value.IntValue RI -> new InterpretResult.CORRECT(
            evalState,
            new Value.IntValue(LI.v().subtract(RI.v())));
        default -> new InterpretResult.INCORRECT(evalState, "Subtraction requires numeric types");
      };
      case Value.BooleanValue LB -> switch (right) {
        case Value.BooleanValue RB -> {
          ...
          yield new InterpretResult.CORRECT(
              evalState,
              new Value.IntValue(leftNum.subtract(rightNum)));
        }
        default -> new InterpretResult.INCORRECT(evalState, "Subtraction requires numeric types");
      };
      default -> new InterpretResult.INCORRECT(evalState, "Subtraction requires numeric types");
