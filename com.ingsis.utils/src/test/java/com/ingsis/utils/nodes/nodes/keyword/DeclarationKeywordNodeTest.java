...
public class DeclarationKeywordNodeTest {
    ...
    public void setUp() {
        ...
        TypeAssignationNode typeAssign = new TypeAssignationNode(id, t, 1, 1);
        LiteralNode lit = new LiteralNode("1", 1, 1);
        ValueAssignationNode valueAssign = new ValueAssignationNode(id, lit, 1, 1);
        decl = new DeclarationKeywordNode(typeAssign, valueAssign, true, 1, 1);
    }

    @Test
    public void acceptMethodsDelegate() {
        assertEquals("checked", decl.acceptChecker(SIMPLE_CHECKER).result());
        assertEquals("interpreted", decl.acceptInterpreter(SIMPLE_INTERPRETER).result());
        assertEquals("visited", decl.acceptVisitor(SIMPLE_VISITOR).result());
    }
}
